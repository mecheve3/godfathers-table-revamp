...
interface HowToPlayModalProps {
  onClose: () => void
  startChapter?: number
  /** Label for the final-chapter "finish" button — "Go to Match" makes sense when this
   *  opens right before a match starts, but reads oddly opened standalone (e.g. from the
   *  home screen), where callers should pass a plain "Done" instead. */
  finalLabelKey?: string
}

export function HowToPlayModal({ onClose, startChapter = 0, finalLabelKey = "howto.nav.start" }: HowToPlayModalProps) {
  const { t } = useLang()
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-t border-[#C9A84C]/20 flex-shrink-0 gap-3">
          ...
          <button
            ...
            style={{ color: '#F5AC0E' }}
          >
            {isLast ? t(finalLabelKey) : t("howto.nav.next")}
